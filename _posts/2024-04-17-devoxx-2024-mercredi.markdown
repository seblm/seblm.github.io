...
### [Multi-Cloud & On Premise, dépassons les frontières ! Tirez le meilleur de chacun des mondes.] par Sébastien Thomas et Francis Besson

Les speakers nous ont présenté leur développement interne qui consiste à abstraire les services cloud utilisés par les
applications. L’intérêt est de décrire de manière standardisée la manière dont un composant sera déployé dans le cloud.
Ils n’ont toutefois pas évoqué les écueils majeurs rencontrés dans ce genre de solution multi-cloud : le coût de
transfert des données d’un cloud provider à l’autre, le fait que tous les services cloud ne se valent pas en terme de
fonctionnalité ou même l’overhead engendré par cette couche d’abstraction maison. J’ai donc été peu convaincu par cette
présentation.

### [La recherche sous stéroïdes - une histoire de sémantique] par Mathilde Rigabert et Martin Labenne

Cette présentation était assez instructive car elle faisait le point sur les différences entre la recherche lexicale
_classique_ qui fonctionne avec des mots clés et la recherche dite sémantique qui permet de rapprocher les concepts et
trouver des synonymes. Dans l’application jolimoi qui est un store de produits de beauté, on a eu la démonstration que
ce qui fonctionnait particulièrement bien est la conjonction de ces deux types de recherche avec un rapprochement dit du
_Reciprocal Rank Fusion_ : en gros trouver un tri pertinent sur des résultats de recherche remontant leur propre score.
L’implémentation de la recherche sémantique s’est faite au moyen d’un moteur trouvé sur Hugging Face et d’une base de
données vectorielle sur Elastic Search. Les métriques qui conditionnait l’amélioration du moteur de recherche était de
mesurer à quel point les utilisateurs reformulaient plusieurs fois de suite ou pas leur recherche. Le dernier point
intéressant est que la recherche sémantique permet de faciliter la mise en place d’un agent conversationnel basé sur un
LLM dans la mesure où les documents les plus pertinents pour une questions peuvent alimenter le contexte de manière
ciblée.

### [Réduisez votre coût d’entrée avec Nix] par Théophane Hufschmitt

Cette dernière conférence de la journée a été l’occasion d’une introduction à Nix pour faciliter la mise en place rapide
et reproductible d’un environnement de développement pour une équipe. Théophane a par la suite étendu son propos en nous
parlant d’autres usages de Nix : de la CI à la distribution Linux.
...
[Multi-Cloud & On Premise, dépassons les frontières ! Tirez le meilleur de chacun des mondes.]: https://www.devoxx.fr/schedule/talk/?id=75801
[La recherche sous stéroïdes - une histoire de sémantique]: https://www.devoxx.fr/schedule/talk/?id=40289
[Réduisez votre coût d’entrée avec Nix]: https://www.devoxx.fr/schedule/talk/?id=20256